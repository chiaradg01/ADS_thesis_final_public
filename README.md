...
├── baseline1_plain.py         # Baseline 1: zero-shot single-agent
...
## Installation
This project was developed using **Python 3.12.13**.

1. Clone the repository:
```bash
   git clone https://github.com/chiaradg01/ADS_thesis_final_public.git
   cd ADS_thesis_final_public
```
...
4. Download the required spaCy model (used for hedging score computation):
```bash
   python -m spacy download en_core_web_sm
```

5. (For local Llama inference only) Install [Ollama](https://ollama.com) and create the
   custom 32k-context model used in this project:
```bash
   echo "FROM llama3.1:latest" > Modelfile
   echo "PARAMETER num_ctx 32768" >> Modelfile
   ollama create llama3.1-32k -f Modelfile
```

6. Create a `.env` file in the project root with your API credentials.
...
### `EDA_wrangling_code_thesis.ipynb`
